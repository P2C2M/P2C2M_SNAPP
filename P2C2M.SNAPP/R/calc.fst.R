...
calc.fst <- function(xml_file, save_graphs){

  #read in file with sample info (tab delimited file called samples.txt)
  sample<-utils::read.table("samples.txt", sep="\t", fill=T, header=F)
  ...
  pop_combo_matrix <- as.data.frame(utils::combn(1:n_pops, 2)) # get all combinations of pairwise population comparisons
  ...
  pair_fst_viols <- id_pairwise_fst(pop_combo_list, empirical_fst, sim_fst_list, save_graphs) # identify populations sharing genes

  fst_result_list <- list(pair_fst_viols) # combine results to list
