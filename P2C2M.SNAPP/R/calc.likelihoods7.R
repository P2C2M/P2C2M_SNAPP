...
calc.likelihoods <- function(post_samples, save_graphs){

  snap_table <- utils::read.table("snap.log", header = TRUE) # read in likelihood table for empirical snapp run
  logs <- Sys.glob("*.log") # get simulated log files
  logs <- logs[which(logs != "snap.log")]
  log_tables <- lapply(logs, function(x) utils::read.table(x, header = TRUE)) # read in simulated likelihood tables
  ...
  emp_sd <- stats::sd(snap_table$likelihood)
  pred_sd <- lapply(like_logs, stats::sd)
  ...
  results_like <- list(p_violations_like_sd)
  return(results_like)
}
