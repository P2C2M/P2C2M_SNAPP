...
id_pairwise_fst <- function(pop_combos, empirical_fst, sim_fst_list, save_graphs){

  sim_pop_fst <- lapply(seq(1, length(empirical_fst)), function(x) lapply(sim_fst_list, function(y) y[x])) # create list where each entry is a pairwise Fst
  ...
  if (save_graphs == TRUE){
    ...
    graphics::barplot(unlist(pair_fst_diff), main = "Difference in Pairwise Fst", xlab = "Pairwise Comparison", names.arg = pop_combo_names, las = 2)
    grDevices::dev.off() # close pdf
  }

  viols <- unlist(pop_combo_names)[which(unlist(pair_fst_diff) == grDevices::boxplot.stats(unlist(pair_fst_diff))$out)] # ID which populations are violating the model by calculating outliers
