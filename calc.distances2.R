...
calc.distances <- function(trees, post_samples, pops, dist_reps){
  ...
  sim_trees <- lapply(lapply(seq(1, length(sim_trees), 1), function(x) sim_trees[x]), function(x) read.tree(text = unlist(x))) # create list of simulated trees by reading into ape
  ...
  for (i in 1:length(sim_trees)){
    ...
    suppressWarnings(distribution_rf <- list(mapply(function(x,y) dist.topo(sim_trees[[i]][x], sim_trees[[i]][y], "PH85"), tree1, tree2))) # calculate distance distributions for simulated posterior
    ...
    suppressWarnings(distances_rf <- suppressWarnings(paste0(lapply(rand_tree, function(x) dist.topo(x, snapp_tree, "PH85"))))) # calculate distances between posterior tree and random trees
  ...
  return(list(p_violations_kf, p_violations_rf))
}
